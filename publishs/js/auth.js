                ...
                if (user) {
                    try {
                        ...
                        if (doc.exists) {
                            ...
                            // Kullanıcının tema tercihini localStorage'a kaydet
                            if (userData.theme) {
                                localStorage.setItem('theme', userData.theme);
                                const appearanceData = JSON.parse(localStorage.getItem('appearanceData')) || {};
                                appearanceData.theme = userData.theme;
                                localStorage.setItem('appearanceData', JSON.stringify(appearanceData));
                                console.log('Kullanıcının tema tercihi localStorage\'a kaydedildi:', userData.theme);
                            } else {
                                // Tema tercihi yoksa varsayılan olarak açık tema kullan
                                localStorage.setItem('theme', 'light');
                                const appearanceData = JSON.parse(localStorage.getItem('appearanceData')) || {};
                                appearanceData.theme = 'light';
                                localStorage.setItem('appearanceData', JSON.stringify(appearanceData));
                                console.log('Varsayılan tema (light) localStorage\'a kaydedildi');
                            }
                        }
                    } catch (error) {
        ...
        registerButton.addEventListener('click', async () => {
            ...
            try {
                ...
                const userData = {
                    ...
                    theme: 'light', // Varsayılan tema ayarı
