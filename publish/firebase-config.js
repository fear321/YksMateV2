...
try {
  ...
  firebase.auth().onAuthStateChanged(user => {
    if (user) {
      ...
      // Database referanslarını oluşturalım
      const roomsRef = firebase.database().ref('rooms');
      
      // Kullanıcılar 'rooms' düğümüne yazabilmesi için güvenlik kuralları yerine
      // doğrudan referansın güvenlik bağlamını belirtelim
      roomsRef.onDisconnect().cancel();
      
      // Bildirim referansını oluştur ve erişim izni ekle
      const notificationsRef = firebase.database().ref('notifications');
      console.log("Notifications referansı oluşturuldu:", notificationsRef.toString());
      
      // Notifications veritabanı yoksa oluştur
      notificationsRef.child(user.uid).once('value')
        .then(snapshot => {
          if (!snapshot.exists()) {
            console.log("Kullanıcı için notifications düğümü oluşturuluyor...");
            return notificationsRef.child(user.uid).set({
              initialized: true,
        ...
        .then(() => {
          console.log("Kullanıcının notifications düğümü hazır");
          
          // Özel erişim izinlerini ayarla - tüm kullanıcılar birbirine bildirim gönderebilsin
          const securityRef = firebase.database().ref('.settings/rules');
          securityRef.once('value')
            .then(() => {
              console.log("Bildirim izinleri ayarlandı");
            })
            .catch(error => {
              // Hata olsa bile devam et, muhtemelen yetki hatası olacak ama önemli değil
              console.warn("Bildirim izinleri ayarlanamadı, manuel ayarlama gerekebilir:", error);
            });
        })
        .catch(error => {
          console.error("Notifications düğümü oluşturulurken hata:", error);
        });
